...
c.apply_idrive_event(evt("AUX_SWAP", mode="MEDIA"))
...
c = new()
c.apply_idrive_event(evt("VOL_UP", 4, mode="MEDIA"))
check(c.state.idrive_detents == 4, "volume up advances the ring")
c.apply_idrive_event(evt("VOL_DOWN", 6, mode="MEDIA"))
...
c.apply_idrive_event(evt("MUTE", mode="MEDIA"))
...
for a in ("VOL_UP", "VOL_DOWN", "MUTE", "NEXT", "PREV"):
    c.apply_idrive_event(evt(a, mode="MEDIA"))
check(c.commands == [], "no HVAC command issued for any media action")
check(c.state.idrive_mode == "media", "but the mirror knows the mode")
