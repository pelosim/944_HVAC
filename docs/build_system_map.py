...
graph LR
  ...
  BRIDGE["Dash Bridge<br/>ESP32-S3"]
  TSPI["TSDash Pi<br/>TunerStudio dash"]
  MS3["MS3-Pro EVO"]
  ...
  LKNOB -- "ESP-NOW" --> LOUT
  LOUT --> LEDS
  PI -- "USB serial" --> BRIDGE
  BRIDGE -- "USB HID keys" --> TSPI
  MS3 -- "FTDI serial" --> TSPI
  ...
  class IDR,LOUT,LKNOB,BRIDGE esp
  class PI,TSPI pi
  class HVAC,LEDS,RADIO,KNOB,PANEL,AUX,MS3 hw
</pre>
  </div>
  <div class="note">
    ...
    <div class="note stop" style="margin-top:14px">
      <p><strong>Open follow-up: enable SSH on the TSDash Pi.</strong> It is the only
      machine in the car with no recovery path. The backup kit in
      <code>944S-PiDash-Backup</code> holds the <code>.tsproj</code>, the project config
      and the dash generator, but it cannot reach the SD image or the TSDash app's own
      configuration over VNC — so if that card dies, the dash comes back only by being
      rebuilt by hand.</p>
      <p>It would also make the Pi deployable the way <code>pi944</code> is, instead of
      being hand-driven through a VNC session. Turning it on needs someone at that
      console: <code>raspi-config</code> &rarr; Interface Options &rarr; SSH, or
      <code>sudo systemctl enable --now ssh</code>, then an alias and a key.</p>
      <p>Nothing is blocked on it. The dash bridge was built specifically so that Pi
      needs nothing installed on it, and that stays true either way.</p>
    </div>

    <p><strong>The TSDash Pi is a second Pi, not a screen on this one.</strong> It runs
    TSDash Echo against the MS3-Pro EVO on its own FTDI serial link, has only VNC open
    (no SSH), and is not networked to the HVAC Pi in the car. That isolation is why the
    dash bridge exists: it reaches that Pi as a USB keyboard, which needs nothing
    installed on the far end.</p>
  </div>
</section>
...
<section id="modules">
  ...
  <div class="grid">
    ...
    <div class="mod">
      <p class="mod-name">Dash Bridge</p>
      <p class="mod-role">USB HID keyboard &rarr; TSDash Pi</p>
      <dl class="kv">
        <dt>Board</dt><dd>Lonely Binary ESP32-S3 N16R8</dd>
        <dt>MAC</dt><dd>3C:DC:75:40:0B:D8</dd>
        <dt>USB serial</dt><dd>3CDC75400BD8 · 303a:1001</dd>
        <dt>Core</dt><dd>esp32 3.3.10 · USBMode=default</dd>
        <dt>Version</dt><dd>dash_bridge v1.1.0</dd>
        <dt>Repo</dt><dd>pelosim/idrive-controller</dd>
      </dl>
    </div>

    <div class="mod pi">
      <p class="mod-name">TSDash Pi</p>
      <p class="mod-role">TunerStudio dash · MS3-Pro EVO</p>
      <dl class="kv">
        <dt>Host</dt><dd>tsdash.local · 192.168.1.93</dd>
        <dt>MAC</dt><dd>2C:CF:67:7E:D7:D7</dd>
        <dt>App</dt><dd>TSDash Echo · 3-seat licence</dd>
        <dt>ECU</dt><dd>MS3-Pro EVO · FTDI AQ03DW41<br/>ttyUSB0 115200 8N1</dd>
        <dt>Access</dt><dd>VNC 5900 only &mdash; no SSH<br/><b>follow-up: enable it</b></dd>
        <dt>Repo</dt><dd>pelosim/944S-PiDash-Backup</dd>
      </dl>
    </div>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><span class="chip t-usb">USB</span></td>
          <td class="mono">built-in FTDI AQ03DW41<br/>ttyUSB0 on that Pi</td>
          <td class="mono">TunerStudio telemetry</td>
          <td>Working</td>
        </tr>
      </tbody>
    </table>
  ...
  <div class="tablewrap">
    <table>
      ...
      <tbody>
        ...
        <tr><td class="mono">/dev/gauges1</td><td class="mono">ttyACM2</td><td>Gauge panel A (master)</td>
    ...
    <div class="note stop">
      <p><strong>The bridge is the one board udev cannot pin by MAC.</strong> Every other
      board here is reached over its <em>native</em> port, where the ESP32-S3 builds the
      USB descriptor itself and publishes its MAC as the serial string. The bridge is
      reached over its <em>UART</em> port, where the host enumerates a CH340 — a separate
      chip with a fixed factory descriptor that knows nothing about the ESP32, and on this
      unit carries no serial number at all.</p>
      <p>So <code>/dev/tsdash</code> is pinned to a physical hub port and follows the
      <em>socket</em>, not the board. Label that socket. The firmware reports its MAC in
      every status line and the backend checks it against <code>TSDASH_MAC</code>, which
      turns a moved cable into a log line instead of keystrokes going somewhere
      unintended — worth having, since the bridge is <code>…0B:D8</code> and the lighting
      board is <code>…0B:F0</code>.</p>
    </div>
  </div>
  <div class="note stop">
...
count   detents in this frame, clamped 1..12</pre>
  ...
  <h3>Which button selects which mode</h3>
  <div class="tablewrap">
    <table>
      <thead><tr><th>Button</th><th>Mode</th><th>LED</th><th>Drives</th></tr></thead>
      <tbody>
        <tr><td class="pin">MEDIA</td><td class="mono">RADIO</td><td class="mono">teal</td>
            <td>head unit over IR</td></tr>
        <tr><td class="pin">MENU</td><td class="mono">HVAC</td><td class="mono">amber</td>
            <td>this Pi — top centre, easiest to find by feel</td></tr>
        <tr><td class="pin">MAP</td><td class="mono">ILLUM</td><td class="mono">violet</td>
            <td>lighting board</td></tr>
        <tr><td class="pin">NAV</td><td class="mono">GAUGE</td><td class="mono">ice blue</td>
  ...
  <div class="note">
    <p><strong>GAUGE and TSDASH are two different screens.</strong> GAUGE is the backup
    cluster built from the T-Display panels; TSDASH is the TunerStudio Pi. They share no
    transport and no actions, and the mode colours are deliberately far apart —
    telling them apart at a glance while driving is the whole job of that LED.</p>
...
<b>{"src":"illum","ch1":200,"ch2":128,"color":3,"relay":1,"night":0}</b></pre>
  ...
  <h3>Pi &rarr; dash bridge · text out, JSON back</h3>
  <pre class="proto"><b>D NEXT</b>   Ctrl+Right   dash to the right
<b>D PREV</b>   Ctrl+Left    dash to the left
<b>D CFG</b>    Ctrl+Up      configuration screen
<b>D HOME</b>   Ctrl+Down    back to main dash
<b>D PING</b> / <b>D GET</b>          no keystroke — status only

<b>{"src":"dash","ok":1,"cmd":"NEXT","q":0,"sent":13,"drop":0,
 "init":1,"usb":1,"mac":"3C:DC:75:40:0B:D8"}</b>
...
mac   which board this is; udev cannot pin it, so it says so itself</pre>
  <div class="note">
    <p><strong>One keystroke per command, never a burst.</strong> The knob clamps rotation
    to 1..12 detents per frame, but the backend ignores <code>count</code> here. TS Dash
    has a short list of tabs and the bridge types blind — it cannot read back which dash
    is showing, so an overshoot is not recoverable. Deliberately slower than the knob
    can turn.</p>
    <p>The keyboard shortcuts are TS Dash's own, from its Getting Started documentation,
    and were confirmed by hand on the TSDash Pi before any of this was built.</p>
  </div>
...
<section id="bench">
  ...
  <ol class="steps">
    ...
    <li><b>iDrive on CAN, bench-powered</b>
      ...
      to wake on the bench — see the open item below before treating that as a fault.</span>
    ...
    <li><b>Dash bridge — UART side first</b>
      <span>Flash over the board's UART port with <code>USBMode=default</code>. Nothing
      needs to be attached to the native port yet.</span>
      <span class="expect">D PING answers · init:1 · mac 3C:DC:75:40:0B:D8</span></li>
    <li><b>Dash bridge — HID side</b>
      <span>Native port into the TSDash Pi on a <em>data-capable</em> cable. Confirm it
      enumerates before trying to type.</span>
  ...
  <div class="note stop">
    <p><strong>Open — bench auto-wake needs a live BMW CAN capture.</strong> The knob still
    requires one physical button press before it starts talking on the bench. The firmware
    sends the whole documented wake set — <code>0x202:FD</code> twice at boot, then
    <code>0x0AA</code> / <code>0x130</code> / <code>0x440</code> / <code>0x560</code> at
    20 ms and <code>0x563:63</code> at 1 s — and the wake loop gives up after 10 s with
    <em>"Wake timeout — physical button press required"</em>. Everything works normally
    after that press, so this blocks unattended bench sessions, not function.</p>
    <p>The suspect is the <code>0x5E7</code> &rarr; <code>0x273</code> init handshake. Our
    <code>0x273</code> reply is a guess: <code>0x5E7</code> byte 4 never reaches
    <code>0x01</code> (fully initialised) without a real car on the bus, so the controller
    never considers itself woken and waits for a human instead.</p>
    <p><strong>The capture to take.</strong> Sniff a running F-series — the 2014 428i — with
    <code>canclaude</code> on the K-CAN, and log every <code>0x5E7</code> alongside the
    <code>0x273</code> the real CIC/head unit answers with, from key-on through the moment
    the knob lights up. Byte-for-byte payload and the reply latency are both wanted; ID
    presence alone will not settle it. Same session, grab whatever else appears before the
    first <code>0x25B</code> — the true wake trigger may be a frame we are not sending at
    all rather than a malformed reply.</p>
    <p><strong>Do not send <code>0x440</code> with byte 1 = <code>0xFF</code></strong> while
    experimenting. That is the sleep request, and it puts the controller down faster.</p>
  </div>
</section>
  ...
  <div class="tablewrap">
    <table>
      ...
      <tbody>
        ...
        <tr><td>An ESP32 powers up, its LED runs, it answers on UART — but the host
            never sees it. No new USB device at all.</td>
            <td><b>Charge-only USB-C cable.</b> VBUS and ground, no D+/D−. Cost an
            evening on 2026-07-31. The board looks perfectly healthy because it
            <em>is</em> — it just has no data path. Anything that shipped with a power
            bank or charger is a suspect; mark the bad one the moment you find it.</td></tr>
        <tr><td>Same symptom, and you cannot tell whether the firmware or the cable
            is at fault</td>
            <td>Ask the board. <code>D GET</code> reports <code>init</code> and
            <code>usb</code> separately: <code>init:0</code> is a firmware or build
            problem, <code>init:1</code> with <code>usb:0</code> means the stack is fine
        ...
        <tr><td>macOS opens "Keyboard Setup Assistant" when the bridge is plugged in</td>
            ...
            beside left Shift and this device only ever sends Ctrl+arrows, so it can
            never be satisfied. Leaving it open can swallow keystrokes.</td></tr>
...
<section id="harness">
  ...
  <div class="note">
    <p><strong>The pot lives inside the actuator, and that has two consequences.</strong>
    It measures actuator shaft travel, so the end stops it reads are the actuator's own —
    which is why <code>flap-pulse.py sweep</code> is safe to run and why its numbers are
    repeatable: nothing can be driven past a stop the feedback is mechanically part of,
    and the calibration survives any work on the door or its linkage without a re-sweep.</p>
    <p>The flip side is that position is <em>actuator</em> position, never <em>door</em>
    position. A broken retainer or a detached linkage leaves the feedback reading
    confidently and correctly while the door does not move at all. Nothing in software
    can see that — it is one of the few faults in this system that only a hand on the
    flap will find.</p>
  </div>
...
<section id="addresses">
  <div class="sechead"><span class="n">10</span><h2>Address registry</h2></div>
  <p>Every identity in the system, in one place. The ESP32-S3 reports its MAC as
  its USB serial, so the same string identifies a board on the bench, on the hub,
  and over ESP-NOW — which is what makes a single registry possible at all.</p>
  <p>One exception, and it is the reason the dash bridge row reads differently: that
  board is reached over its <em>UART</em> port, so the host sees a CH340 rather than the
  ESP32. Its MAC is real and confirmed, but it reaches the Pi as data in the protocol,
  not as a USB descriptor udev can match.</p>

  <h3>ESP32 boards</h3>
  <div class="tablewrap">
    <table>
      <thead><tr>
        <th>Board</th><th>MAC / USB serial</th><th>Port</th><th>Toolchain</th><th>Firmware</th>
      </tr></thead>
      <tbody>
        <tr><td><b>iDrive controller</b><br/><span class="mono">Lonely Binary N16R8</span></td>
        ...
        <tr><td><b>Lighting output</b><br/><span class="mono">Lonely Binary N16R8</span></td>
            ...
            <td class="mono">pwm_controller</td></tr>
        <tr><td><b>Gauge panel A</b> — master<br/><span class="mono">T-Display-S3-Long #1</span></td>
            <td class="mono">68:EE:8F:48:19:9C</td>
            <td class="mono">/dev/gauges1<br/>/dev/gauges</td>
            <td class="mono">core 2.0.14</td>
            <td class="mono">gauge_bench</td></tr>
        <tr><td><b>Gauge panel B</b> — slave<br/><span class="mono">T-Display-S3-Long #2</span></td>
            <td class="mono">68:EE:8F:48:18:D4</td>
        ...
        <tr><td><b>Lighting knob</b><br/><span class="mono">CrowPanel 1.28"</span></td>
            <td class="mono">not on the hub</td>
            <td class="mono">—</td>
            <td class="mono">core 2.0.14</td>
            <td class="mono">display_controller</td></tr>
      </tbody>
    </table>
  </div>
  <div class="note">
    ...
    tells any of them apart, which is exactly why boards are pinned by serial rather
    than by vendor — adding the hub already reshuffled <code>ttyACM</code> numbering
    once. The bridge is the one that cannot be pinned that way, so the backend checks
    the MAC it reports instead.</p>
  </div>

  <h3>Hosts and services</h3>
  <div class="tablewrap">
    <table>
      <thead><tr><th>Name</th><th>Address</th><th>Notes</th></tr></thead>
      <tbody>
        <tr><td>HVAC Pi — hostname</td><td class="mono">944HVACPi</td>
            <td>ssh alias <code>pi944</code>, user <code>mark</code></td></tr>
        <tr><td>HVAC Pi — eth0</td><td class="mono">192.168.1.142</td>
            <td>DHCP; has moved before — prefer the hostname</td></tr>
        <tr><td>HVAC Pi — wlan0</td><td class="mono">192.168.1.240</td>
            <td>DHCP</td></tr>
        <tr><td>mDNS</td><td class="mono">944HVACPi.local</td>
            <td>unreliable — has cached stale leases and resolves to a dead
                IPv6 record; use <code>ssh -4</code></td></tr>
        <tr><td>TSDash Pi</td><td class="mono">tsdash.local · 192.168.1.93</td>
            <td>MAC <code>2C:CF:67:7E:D7:D7</code>; VNC 5900 only, <b>no SSH</b>.
                Not networked to the HVAC Pi in the car — hence the bridge.</td></tr>
        <tr><td>Dashboard</td><td class="mono">http://944HVACPi.local:8000</td>
            <td>WebSocket at <code>/ws</code>, REST at <code>/api/state</code></td></tr>
      </tbody>
    </table>
  </div>

  <h3>Protocol identifiers</h3>
  <div class="tablewrap">
    <table>
      <thead><tr><th>Bus</th><th>Identifier</th><th>Meaning</th></tr></thead>
      <tbody>
        <tr><td>CAN</td><td class="mono">0x25B</td><td>iDrive — all 14 inputs, 500 kbps</td></tr>
        <tr><td>CAN</td><td class="mono">0x202 / 0x563 / 0x0AA / 0x130 / 0x440 / 0x560</td>
            <td>keep-alives the iDrive needs to stay awake</td></tr>
        <tr><td>CAN</td><td class="mono">0x5E7 &rarr; 0x273</td>
            <td>init handshake — reply format is a guess, and the reason the knob needs a
            button press on the bench. Open: capture it off the 428i (see &sect;07)</td></tr>
        <tr><td>CAN</td><td class="mono">0x440 byte1 = 0xFF</td>
            <td><b>sleep request</b> — never send it</td></tr>
        <tr><td>IR</td><td class="mono">NEC addr 0x00</td>
            <td>cmds 0x02 vol+ · 0x98 vol− · 0xE2 mute · 0x90 next · 0xE0 prev</td></tr>
        <tr><td>ESP-NOW</td><td class="mono">0xAA / 0xBB</td>
            <td>knob&rarr;output / output&rarr;knob, 4-byte packets</td></tr>
        <tr><td>USB HID</td><td class="mono">Ctrl+&rarr; / Ctrl+&larr;<br/>Ctrl+&uarr; / Ctrl+&darr;</td>
            <td>TS Dash: next dash · prev dash · config · main dash</td></tr>
        <tr><td>USB</td><td class="mono">303a:1001</td>
            <td>dash bridge as a HID keyboard, "944S Dash Bridge"</td></tr>
        <tr><td>USB</td><td class="mono">1a86:7522</td>
            <td>the bridge's CH340 UART side — <b>no serial number</b></td></tr>
        <tr><td>I²C</td><td class="mono">0x48</td><td>ADS1115 — on the Pi <em>and</em> on gauge panel A</td></tr>
        <tr><td>1-Wire</td><td class="mono">bd3d51 / be5d11 / bbdd26</td>
            <td>DS18B20 duct / exterior / cabin — no <code>28-</code> prefix</td></tr>
      </tbody>
    </table>
  </div>
  <div class="note stop">
    <p><strong>Panel B has no firmware and must not be flashed with panel A's.</strong>
    <code>slave_app.h</code> is an app layer waiting to be dropped into a copy of
    LilyGo's <code>lvgl_demo</code>; that sketch has not been assembled. Panel A's
    <code>gauge_bench</code> drives an ADS1115 that panel B does not have —
    <code>flash-via-pi.sh gauges2</code> refuses for exactly this reason.</p>
  </div>
</section>
